...
description: "Use on every markdown write under memory/ -- create, rename, move, or delete, at any depth. Patches ancestor CLAUDE.md navigation. Not for files outside memory/."
tags: [memory, claude-md, hierarchy, navigation]
---

# Memory CLAUDE.md sync

Whenever a markdown file under `memory/` changes, the `CLAUDE.md` files above it must stay coherent. This skill keeps that hierarchy honest: every time you create, rewrite, rename, move, or delete a memory page, walk up the directory chain and patch each ancestor `CLAUDE.md`.
...
2. Collect every `CLAUDE.md` between `P`'s directory and `memory/` (inclusive of both). The workspace root `CLAUDE.md` is **not** in scope — it lists top-level workspace folders only and does not need updating when a single memory page changes.
3. Order the chain from closest-to-`P` (the immediate parent) to furthest (`memory/CLAUDE.md`).

The template's `memory/` is **flat**: every page is a sibling of `memory/CLAUDE.md`, so for almost every page the chain is length 1 — `memory/CLAUDE.md` only. A leaf like `memory/foo.md` has just `memory/CLAUDE.md` above it. The multi-level walk-up below still applies if you grow `memory/` into nested sub-folders (each with its own `CLAUDE.md`); for the default flat wiki, Step 2 is usually all you need.

## Step 2 — Update the immediate parent

The immediate parent `CLAUDE.md` is where the changed page is listed by name. In the default flat wiki this is always `memory/CLAUDE.md`. Pick the right section by inspecting which catalog sections exist in that file:
...
| Moved page | Remove the row from the old parent (if the page moved across directories) and insert it in the new parent. Both parents are in scope for the sweep. In a flat wiki a "move" is usually just a section change (e.g. a page reclassified from `Concepts` to `Projects`) within the same `memory/CLAUDE.md`. |
...
This step only applies if `memory/` has nested sub-folders with their own `CLAUDE.md` files. In the default flat wiki the chain ends at `memory/CLAUDE.md` and you can skip ahead to Step 4.
...
1. Find the row in the ancestor's `## Deep-Dive Navigation` table that points to the next step down the chain — e.g. `[sub-area/CLAUDE.md](sub-area/CLAUDE.md)` inside `memory/CLAUDE.md`.
...
- **Use relative links** rooted at the `CLAUDE.md` file being edited (`foo.md` from `memory/CLAUDE.md`, not absolute paths, not `[[wikilinks]]`).
